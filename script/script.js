	...
	function hideSearch(){
		...
		/* if( i < headerMenuEls.length){
			headerMenuEls.each(function(){
				$(this).Delay(i*.9 / headerMenuEls.length).fadeIn()
				i++
			})		
		} */
		// searchDelayEls.each..
...
/* 	const windowWidth = $(window).width();
	const $tclVideo = $("#tcl-video")

	if(windowWidth <= 1024){
		$tclVideo.find(".pcMode").css({display:"none"}).
		$tclVideo.find(".mobileMode").css({display:"block"})
	}else if(windowWidth > 1024){
		$tclVideo.find(".pcMode").css({display:"block"}).
		$tclVideo.find(".mobileMode").css({display:"none"})
	}
 */
